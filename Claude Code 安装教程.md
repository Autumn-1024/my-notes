...
启动后即可正常对话，默认使用配置文件中指定的 MiMo 模型。

> 💡 如果临时想切换模型，可以用 `claude --model 模型名` 覆盖默认配置。
...
### Q: 配置了 MODEL 环境变量但不生效？

**原因**：Claude Code 不识别 `MODEL` 环境变量。

**解决**：必须使用 `ANTHROPIC_MODEL` 作为环境变量名。配置文件示例：

```json
{
  "env": {
    "ANTHROPIC_BASE_URL": "https://token-plan-cn.xiaomimimo.com/anthropic",
    "ANTHROPIC_AUTH_TOKEN": "tp-你的API密钥",
...
3. 创建 ~/.claude/settings.json（填入 BASE_URL + API Key + ANTHROPIC_MODEL）
4. 创建 ~/.claude.json → {"hasCompletedOnboarding": true}
5. 清除官方环境变量（如有）
6. 终端输入 claude 启动，默认使用 MiMo 模型
```
...
Claude Code 在协作模式下有三种运行状态：

### ⚡ 状态一：运行中（Running）

- Claude 在思考、调用工具、写代码、读文件
- 你在旁边看着就行，不用动
- 界面有转圈/动画表示在干活
...
- Claude 需要你输入
- 比如：确认权限弹窗、回答问题、提供信息、选择方案
- 界面有提示，等你操作

### ✅ 状态三：就绪（Ready）

- Claude 完成了当前任务，等你发下一步指令
- 或者报错停下来了，需要你决定怎么办
- 输入框可以打字
